  ...
  <div class="content">
   <el-card shadow="never">
     <el-row>
       ...
       <el-col :span="2">
        <el-select v-model="aliregion" placeholder="选择阿里Region" @change="getAliRedisCluster()">
            <el-option 
              v-for="item in aliredisregion" 
              :key="item.LocalName" 
...
//腾讯操作
const aliregion = ref("")
const aliredisregion = ref<any[]>([])
const alirediscluster = ref<any[]>([])
const fromaliredis = reactive({
  cloud: 'aliredis',
  region: '',
})
// 弹窗
...
const load = async () => {
  let result = (await listCloudRegion(fromaliredis)).data
  if (result.errorCode === 0 ) {
    aliredisregion.value = result.data.region_list
  } else {
    ElMessage.error(result.msg)
  }  
}
