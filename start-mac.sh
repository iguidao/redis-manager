...
rediswebmanager=`docker ps -a | grep redis-web-manager |wc -l`
...
if [ ${rediswebmanager} == 1 ]
   then
      ...
      docker run -d -it -v ./logs/:/data/logs  -p  8000:8000 --restart=always  --name redis-web-manager redis-web-manager
else
   echo "docker run redis-web-manager"
   docker run -d -it -v ./logs/:/data/logs  -p  8000:8000 --restart=always  --name redis-web-manager redis-web-manager
fi
